...
| **Status** | Finalized — Architect Audit Applied |
| **Revision** | V2.0 incorporates architect audit findings A1–A12 from the V1.0 review. All corrections, clarifications, and new sections are marked with **[V2]**. |
...
12. Architect Audit Register (Blueprint Self-Review) **[V2]**
...
> **[V2] OIC Platform Constraint — Synchronous Invocation Requirement (Audit A2):**
> A Scheduled integration (schedule-triggered) **cannot** be invoked synchronously by another integration in OIC. Only integrations with a **REST trigger** or **SOAP trigger** can be invoked synchronously as child integrations.
>
> Therefore, domain integrations invoked by the coordinator **must use REST triggers** (type `IFS_`, not `IFP_`). Only the coordinator itself (`IFP_COORD_HCM_DAILY`) has a schedule trigger. The domain integrations are REST-triggered callable integrations that the coordinator invokes via the OIC REST adapter in synchronous request/reply mode.
>
> **Alternative approach (not recommended):** The coordinator could submit schedule-triggered domain integrations via the OIC REST Management API and poll for completion. This loses the simplicity of synchronous invocation and introduces additional polling logic in the coordinator itself.
...
> **[V2] DVM Type Clarification (Audit A3):** OIC Domain Value Maps store **all values as strings**. There is no Integer, Boolean, or typed column in a DVM. The `MaxPollCount` column stores a numeric value as a string. The integration must cast it to a number using `number($var_MaxPollCount)` in the WHILE condition XPath for arithmetic comparison. The default value `'36'` in the DVM lookup expression provides a string fallback that is also cast via `number()`.
...
```text
 ...
 Step 5a  [V2] SOURCE FILE VALIDATION (Audit A12):
 ...
 Step  6  [If source IDs needed] IFS_BIP_SRCID_PREFETCH
                                  → bulk BIP lookup, returns XML payload (or stage path)
...
2. RETURN: XML result as response payload (for small datasets)
   — or —
...
> **[V2] BIP Parameter Size Constraint (Audit A5):** BI Publisher SOAP `runReport` parameters are transmitted within the SOAP body. For files with 1,000+ person numbers, the `personNumberList` parameter can become very large. Additionally, if the BIP report SQL uses an Oracle `IN` clause, Oracle Database limits `IN` lists to 1,000 items.
>
> **Recommended limits:**
>
> - Maximum `personNumberList` size: **1,000 person numbers per call**
> - For files exceeding 1,000 records: invoke `IFS_BIP_SRCID_PREFETCH` in a chunked loop (batches of 1,000), aggregating the XML results before the main FOR loop
> - The BIP report SQL should use a table function or `XMLTABLE` to parse the comma-delimited input rather than a raw `IN` clause
>
> **[V2] Source ID XML Access Pattern (Audit A7):** The pre-fetched source ID XML must be accessible inside the domain integration's FOR loop. Two implementation options:
>
> - **Option A (recommended for smaller datasets):** `IFS_BIP_SRCID_PREFETCH` returns the XML as the **REST response payload**. The calling integration assigns the response to a flow variable (`$srcIdDoc`). XPath key lookup inside the FOR loop: `$srcIdDoc/SourceSystemIDs/Person[PersonNumber = $currentPersonNumber]/WR_SourceSystemId`. OIC flow variables have a size limit (typically ~5 MB depending on OIC version); this option is suitable for up to ~5,000 employee records.
> - **Option B (for large datasets):** The sub-integration writes the XML to a stage file. The calling integration performs one `STAGE_READ` before the FOR loop to load the XML into a flow variable. If the XML exceeds the flow variable size limit, the integration must fall back to per-record BIP calls or implement a chunked approach.
...
For complex integrations with multiple HDL object types, two implementation patterns are available. Choose based on the integration's complexity and whether conditional per-record logic is required.

#### Option A: Single-Pass XSL Transformation (Preferred When No Per-Record Conditional Logic)

Apply a single XSLT map to the **entire staged source CSV** (read as XML by the OIC Stage File adapter) to produce each HDL section in one transform. No FOR loop is required — the XSL handles all record iteration internally.
...
**One STAGE_WRITE per HDL section** writes the XSL output to the staging file.
...
- No FOR loop overhead — OIC XSL engine processes all records in a single pass

#### Option B: FOR Loop with STAGE_WRITE Append (Required When Per-Record Logic Is Needed)

> **[V2] OIC Platform Constraint (Audit A4):** OIC App-Driven Orchestration does **not** support incrementally appending XML elements to a flow variable inside a FOR loop. OIC assignment operations are atomic replacements — there is no `$xml = $xml + <newElement>` concatenation pattern.

When the integration requires per-record conditional logic (e.g., Action-code routing, source ID lookup per record, conditional field derivation), use the FOR loop with **STAGE_WRITE in append mode**:

```text
FOR each record:
  STAGE_READ → read record fields
  ROUTER (Action?) → determine which HDL objects apply
  STAGE_WRITE (append=true) → append pipe-delimited HDL line(s) to the appropriate .dat file
```

Each `.dat` file is opened with a METADATA header row written **once before the FOR loop** (or using a header-flag variable on first iteration). Subsequent iterations append MERGE rows.

> **Recommendation:** Prefer Option A for integrations where all records follow the same HDL mapping (e.g., JOB, DEPARTMENT). Use Option B for integrations with Action-code branching (e.g., EMPLOYEE with HIRE/REHIRE/TERMINATION).

---

### 4.7 Re-Run Safety and Idempotency **[V2]**

> **[V2] Added per Audit A11.**

The `MERGE` HDL action (Section 4.4) provides **record-level idempotency** — re-processing the same employee record produces the same result in Oracle HCM. However, this does not prevent duplicate ESS job submissions at the integration level.

**Scenario:** The integration fails AFTER `importAndLoadData` succeeds but BEFORE the SFTP archive step. On re-run, the source file is still present on SFTP and will be submitted to HDL again, creating a duplicate ESS job.

**Mitigation strategies (implement at least one):**

| Strategy | Implementation | Trade-off |
| --- | --- | --- |
| **SFTP marker file** | After successful `importAndLoadData`, write a zero-byte marker file (e.g., `{FileName}_{YYYYMMDD}.submitted`). Before Step 3 (ListFile), check for the marker. If present, skip submission or resume from the polling step. | Requires marker cleanup on successful completion |
| **Audit log check** | Before SFTP download, query the SFTP audit log (Section 9.1) for a matching `SourceFile` + today's date with `FinalStatus != FAULT`. If found, skip. | Depends on audit log being written even on partial success |
| **Accept duplicate MERGE** | Document that duplicate `MERGE` submissions are safe and accept the duplicate ESS job. Monitor for duplicate runs via the audit log. | Simplest; acceptable if HDL queue capacity is not a concern |

> The recommended approach for most implementations is the **SFTP marker file** because it requires no external dependency and can be checked before any SFTP download occurs.

---

### 4.8 Source File Validation **[V2]**

> **[V2] Added per Audit A12.**

After SFTP DownloadFile (Step 5) and before the FOR loop or XSL transform, validate the source file:
...
| Record count within ceiling | Count records from STAGE_READ total | If > 10,000: log warning in activity stream. Oracle recommends ≤ 10,000 records per single HDL batch for reliable performance. For larger files, consider chunked submission. |
...
> **`COMPLETED` is not a documented ESS terminal status for HCM Data Loader jobs** based on Oracle's HCM Data Loader documentation. Do not include it in any WHILE loop condition. The documented terminal statuses for HDL ESS jobs are the five listed above (SUCCEEDED through BLOCKED). **[V2] (Audit A10):** If your Oracle HCM release introduces changes to the ESS status model, verify against Oracle Support Doc ID or the `getESSJobStatus` WSDL response enumeration for your specific HCM version.
...
> **[V2] XPath Syntax Correction (Audit A9):** The XPath logical AND operator is `and` (lowercase). `AND` is not valid XPath syntax. While some OIC expression parsers may be case-insensitive, always use the correct XPath specification form.
>
> **[V2] Activity Stream Logging in WHILE Loop (Audit A6):** Do **not** log an activity stream entry on every poll iteration. OIC has a limit on activity stream entries per instance (typically 100–200 entries visible in the monitoring console). For a 36-iteration poll cycle, per-iteration logging would consume 36 entries — potentially pushing earlier diagnostic milestone entries out of view.
>
> **Recommended polling log strategy:**
>
> - Log on **first poll** (confirms polling has started): `{"step":"ESS_POLL_START","essJobId":"{essJobId}","maxPolls":"{maxPolls}"}`
> - Log on **loop exit** (already covered by Milestone #3 in Section 7.2): `{"step":"ESS_COMPLETE","essJobId":"{essJobId}","essStatus":"{HDLJobStatus}","pollCount":"{pollCount}"}`
> - Optionally: log every **10th iteration** for long-running jobs to provide intermediate visibility
>
> This reduces worst-case polling log entries from 36 to 2–4 while preserving full diagnostic value.

---

### 5.3 ESS Service Binding — HCM Cloud Adapter Standard

All HCM FBDI integrations use the **HCM Cloud Adapter** to invoke the `getESSJobStatus` operation. The HCM Cloud Adapter exposes this operation through the Oracle `ErpIntegrationService` service catalog — this is the standard and correct mechanism.

> **[V2] Correction (Audit A1):** The V1 blueprint incorrectly stated that `ErpIntegrationService.getESSJobStatus` was accessed "via the SOAP adapter" in IFR_EMPLOYEENA and that the other 3 integrations used a "different" HCM-native service. **This was factually incorrect.** JCA artifact analysis confirms that ALL four reference integrations use the same configuration:
>
> - Adapter type: `adapter="hcm"` (HCM Cloud Adapter)
> - Service: `{...erpIntegrationService/}ErpIntegrationService`
> - Operation: `getESSJobStatus`
> - Operation path: `http://xmlns.oracle.com/apps/financials/commonModules/shared/model/erpIntegrationService/getESSJobStatus`
>
> The HCM Cloud Adapter surfaces `ErpIntegrationService` operations (including `getESSJobStatus`) through its built-in service catalog. This is **not** the same as configuring a standalone SOAP adapter with a separate WSDL. All ESS polling invocations go through the single `{PREFIX}_HCM` connection.

**Naming consistency rule:** When configuring the HCM adapter's `getESSJobStatus` invocation, use a consistent JCA reference name across all integrations (e.g., `GetEssJobStatus`). Do not use variant names like `getCurrentESSStatus` — this creates confusion during maintenance without any functional difference.
...
> **Implementation note:** Some OIC versions may restrict tracking field expressions to simple variable references only (not function calls). If `fn:format-dateTime` is not accepted in the tracking panel, pre-compute the date string in an ASSIGNMENT node (`$var_RunDate`) and reference `$var_RunDate` as the tracking expression instead.
...
>
> **[V2] Activity Stream Budget (Audit A6):** OIC limits the number of visible activity stream entries per instance (typically 100–200). Budget your activity stream entries carefully:
>
> - 6 milestone entries (mandatory, listed above)
> - 1–2 ESS poll entries (first poll + loop exit — see Section 5.2)
> - Per-record RECORD_SKIP entries (Section 4.1 Step 7) — only for skipped/unknown actions, not for every processed record
> - Fault handler entries (Level 1/2/3 — Section 6.1)
>
> For integrations processing large files with many skipped records, consider logging a **summary count** of skipped records after the FOR loop rather than one entry per skipped record.
...
> **Note on `&amp;` in XPath:** The `&amp;` entity appears in the XPath expression because OIC stores XPath expressions in XML files (`expr.properties`) where `&` must be XML-encoded. At runtime, the XPath engine evaluates this as a literal `&` character in the URL string — which is correct for the URL itself. When this URL is inserted into an HTML notification body, the `&` in the URL must again be represented as `&amp;` in the HTML source to produce a valid hyperlink.
...
> **[V2] Concurrency Note (Audit A8):** Each domain integration writes to its own `/Audit/{Domain}/` directory, providing natural isolation during normal scheduled runs. The OIC FTP Adapter supports Write File in append mode for this pattern. However, concurrent writes to the same audit file (e.g., during manual re-runs of the same domain) could cause corruption depending on the SFTP server's file-locking behavior. **Recommendation:** Sequence manual re-runs of the same domain (do not run two instances of the same integration concurrently). For additional safety, configure the SFTP server to use file-level locking for the `/Audit/` directory.
...
| 13 | **[V2]** ESS JCA naming | Review all `getESSJobStatus` JCA files | Consistent `referenceName` across all integrations |
| 14 | **[V2]** Source file validation | Review post-download logic | File size > 0 check exists before processing |
...
| 8 | **[V2]** Re-run safety test | Run integration twice with same file; confirm no data corruption |
...
| D9 | Info | IFR_EMPLOYEENA | ESS polling JCA reference named `getCurrentESSStatus` instead of `GetEssJobStatus` | Inconsistent naming; same adapter/service/operation as other integrations | Section 5.3 |
...
> **[V2] D9 Revision (Audit A1):** D9 was downgraded from Medium to Info. The original V1 description ("Uses `ErpIntegrationService` for ESS polling; other 3 use HCM adapter") was **factually incorrect**. JCA artifact analysis confirmed that ALL four integrations use the HCM Cloud Adapter (`adapter="hcm"`) calling the same `ErpIntegrationService.getESSJobStatus` operation through the HCM adapter's service catalog. The actual defect is limited to an inconsistent JCA reference name (`getCurrentESSStatus` vs `GetEssJobStatus`) — a naming-only issue with no runtime impact.

---

## 12. Architect Audit Register (Blueprint Self-Review) **[V2]**

This section documents the findings from the architect-level audit of Blueprint V1.0. Each finding was verified against extracted IAR artifacts (JCA files, `all_vars.txt`, project metadata) or documented OIC platform capabilities. No speculative items are included.

### Audit Summary

| ID | Severity | Section | Issue Type | Finding | Resolution in V2 |
| --- | --- | --- | --- | --- | --- |
| A1 | Critical | 5.3, D9 | Factual Error | V1 claimed IFR_EMPLOYEENA used a "SOAP adapter" for ESS polling and that other integrations used a "different HCM-native service." All 4 integrations use identical HCM adapter + ErpIntegrationService configuration. | Section 5.3 rewritten; D9 downgraded to Info with corrected description |
| A2 | High | 1.3 | Implementability | Schedule-triggered integrations cannot be invoked synchronously by another OIC integration. The coordinator pattern requires REST-triggered domain integrations. | Section 1.3 updated with OIC constraint callout; naming convention clarified |
| A3 | Medium | 3.1 | Inaccuracy | DVM `MaxPollCount` listed as type "Integer" — OIC DVMs have no typed columns; all values are strings. | Column type corrected to String; `number()` cast requirement documented |
| A4 | High | 4.6 | Implementability | V1 described accumulating XML in a flow variable inside a FOR loop — OIC does not support incremental variable append. | Section 4.6 rewritten with two valid implementation patterns (single-pass XSL vs. STAGE_WRITE append) |
| A5 | Medium | 4.3 | Scalability | BIP `personNumberList` parameter has no documented size limit guidance. Oracle DB IN-clause limit is 1,000 items. | Size limit, chunking strategy, and SQL design requirement added |
| A6 | Medium | 5.2, 7.2 | Platform Limit | Per-iteration ESS poll logging could saturate OIC activity stream (100–200 entry limit per instance). | Polling log strategy changed to first/last only; activity stream budget guidance added |
| A7 | Medium | 4.3 | Ambiguity | `$srcIdDoc` XPath reference mechanism inside FOR loop was unspecified — unclear how stage file content becomes an XPath-navigable variable. | Two implementation options documented with size constraints |
| A8 | Low | 9.1 | Concurrency | Concurrent SFTP audit log appends during manual re-runs could cause file corruption. | Concurrency note and sequencing recommendation added |
| A9 | Low | 5.2 | Syntax | WHILE condition used `AND` (uppercase) — invalid XPath; correct form is `and` (lowercase). | Corrected to lowercase `and` throughout Section 5.2 |
| A10 | Low | 5.1 | Citation | Claim that "COMPLETED is returned for non-HDL job categories" was a positive assertion without citation. | Softened to "not a documented status" with version-verification guidance |
| A11 | Medium | 4.1 | Omission | No integration-level re-run safety design — duplicate HDL submissions possible if integration fails after importAndLoadData but before archive. | New Section 4.7 added with three mitigation strategies |
| A12 | Medium | 4.1 | Omission | No source file validation (size check, header schema, record count ceiling) after download. | New Section 4.8 added with validation checks and HDL batch size guidance |

### Audit Evidence References

| Audit ID | Evidence Source |
| --- | --- |
| A1 | `GetEssJobStatus_REQUEST.jca` (IFP_EMPLO): `adapter="hcm"`, `faServiceQualifiedName="{...erpIntegrationService/}ErpIntegrationService"`. `getCurrentESSStatus_REQUEST.jca` (IFR_EMPLOYEENA): identical `adapter="hcm"` and `faServiceQualifiedName`. |
| A2 | OIC platform documentation: Scheduled integrations expose no invocable endpoint. Only REST/SOAP-triggered integrations can be invoked synchronously by another integration. |
| A3 | OIC DVM editor: all columns stored as `java.lang.String`. No type selector exists in the DVM column definition UI. |
| A4 | OIC App-Driven Orchestration runtime: ASSIGN action replaces variable value atomically. No append/concat-to-XML-variable operation exists. |
| A5 | Oracle Database SQL reference: `IN` clause limited to 1,000 expressions. BIP SOAP body has practical XML payload size constraints. |
| A6 | OIC Monitoring UI: activity stream entries beyond platform limit are not displayed. Exact limit varies by OIC version (Gen2/Gen3). |
| A9 | W3C XPath 1.0 / 2.0 specification: logical operators are `and`, `or`, `not()` — all lowercase. |
...
> | 2.0 | 2026-05-14 | Integration Architecture Team | Architect audit applied — 12 findings (A1–A12) integrated; Section 5.3 corrected, D9 revised, Sections 4.6–4.8 rewritten/added, Section 12 added |
